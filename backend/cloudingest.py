...
def ingest_docs_cloud():
    ...
    # 3. Create/Reset Collection on CLOUD
    print(f"Connecting to Qdrant Cloud at {settings.QDRANT_CLOUD_URL}...")
    client = QdrantClient(
    ...
    if settings.COLLECTION_NAME not in [c.name for c in collections.collections]:
        ...
        print(f"Created cloud collection {settings.COLLECTION_NAME}")

    # 4. Indexes
    embeddings = HuggingFaceEmbeddings(model_name=settings.EMBEDDING_MODEL)
    
    print("Uploading chunks to cloud...")
